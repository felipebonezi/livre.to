 ...
 */
public class MaterialFinder extends AbstractFinder<Material> implements IFinder<Material> {
	...
	public Material selectUnique(Long id) {
		ExpressionList<Material> expressionList = super.generateEqualExpressions(new String[] {ID}, new Object[] {id});
		return expressionList.findUnique();
	}

	@Override
	public Material selectUnique(String[] columns, Object[] columnsArgs) {
		ExpressionList<Material> expressionList = super.generateEqualExpressions(columns, columnsArgs);
	 ...
	 */
	public Page<Material> page(int page, int pageSize, String sortBy, String order, String filter) {
		return getFinder()
			.where()
			.ilike("title", "%" + filter + "%")
	...
	public Page<Material> page() {
		return page(0, 10, "title", "asc", "");
	}
